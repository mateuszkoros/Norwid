...
def check_requirements():
    ...
    if (
        not Path(os.getenv("MAIL_HTML_FILE")).is_file()
        or not Path(os.getenv("MAIL_TXT_FILE")).is_file()
    ):
...
def check_heartbeat():
    heartbeat_time = datetime.datetime.fromtimestamp(
        os.path.getmtime(os.getenv("HEARTBEAT_FILE"))
    )
